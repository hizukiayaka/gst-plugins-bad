...
#include "gstrgasink.h"
#include "gstkmsutils.h"
#include "gstkmsbufferpool.h"
#include "gstkmsallocator.h"

#define GST_PLUGIN_NAME "rgasink"
...
GST_DEBUG_CATEGORY_STATIC (gst_rga_sink_debug);
GST_DEBUG_CATEGORY_STATIC (CAT_PERFORMANCE);
#define GST_CAT_DEFAULT gst_rga_sink_debug
...
static void gst_kms_sink_drain (GstRGASink * self);

#define parent_class gst_rga_sink_parent_class
G_DEFINE_TYPE_WITH_CODE (GstRGASink, gst_rga_sink, GST_TYPE_VIDEO_SINK,
...
{
  GstRGASink *self = GST_RGA_SINK (overlay);
...
static gboolean
cache_conn_properities (GstRGASink * self)
{
  gboolean ret = TRUE;
...
static gboolean
cache_crtc_properities (GstRGASink * self)
{
  gboolean ret = TRUE;
...
static gboolean
cache_plane_properities (GstRGASink * self, gint plane_id)
{
...
static gboolean
cache_planes_properities (GstRGASink * self, drmModePlaneRes * pres)
{
  GData *props = NULL;
  drmModePlane *plane;
  int i;
...
static gint
add_connection_property (GstRGASink * self, drmModeAtomicReq * req,
...
static gint
add_crtc_property (GstRGASink * self, drmModeAtomicReq * req,
    const gchar * name, guint64 value)
{
  gint ret = 0;
  gint crtc_id = self->crtc_id;
...
static gint
add_plane_property (GstRGASink * self, drmModeAtomicReq * req,
    gint plane_id, const gchar * name, guint64 value)
{
  gint ret = 0;
...
static drmModeCrtc *
find_crtc_for_connector (int fd, drmModeRes * res,
    drmModeConnector * conn, guint * pipe)
{
  int i;
  int crtc_id;
  drmModeEncoder *enc;
...
find_main_monitor (int fd, drmModeRes * res)
{
  ...
  static const int priority[] = {
    DRM_MODE_CONNECTOR_LVDS, DRM_MODE_CONNECTOR_eDP
  };
  int i;
  drmModeConnector *conn;
...
static void
log_drm_version (GstRGASink * self)
{
...
static gboolean
configure_mode_setting (GstRGASink * self, GstKMSMemory * kmsmem,
...
static void
atomic_flip_handler (gint fd, guint frame, guint sec, guint usec,
    guint crtc_id, gpointer data)
{
  GstRGASink *self = GST_RGA_SINK (data);
  gst_buffer_replace (&self->last_buffer, NULL);
}

static gboolean
gst_kms_sink_wait_event (GstRGASink * self)
{
  gint ret;
...
static void
gst_kms_sink_update_connector_properties (GstRGASink * self,
...
static void
gst_kms_sink_update_plane_properties (GstRGASink * self, gint plane_id,
    drmModeAtomicReq * req)
{
  SetPropsIter iter;
...
#if 0
static GstCaps *
gst_kms_sink_get_allowed_caps (GstRGASink * self)
{
...
gst_kms_sink_get_caps (GstBaseSink * bsink, GstCaps * filter)
{
  ...
  out_caps = gst_kms_sink_caps_template_fill ();
...
static void
ensure_kms_allocator (GstRGASink * self)
{
  if (self->allocator)
    return;
...
gst_kms_sink_create_pool (GstRGASink * self,
...
static gboolean
gst_kms_sink_check_scale (GstRGASink * self, GstVideoInfo * const vinfo)
{
  gint ret;
  drmModeAtomicReqPtr req = NULL;
...
static gboolean
gst_kms_sink_calculate_display_ratio (GstRGASink * self, GstVideoInfo * vinfo)
{
  guint dar_n, dar_d;
...
gst_kms_sink_set_caps (GstBaseSink * bsink, GstCaps * caps)
{
  ...
  gst_video_info_set_format (&conv_vinfo, GST_VIDEO_FORMAT_RGBx,
      GST_VIDEO_INFO_WIDTH (&vinfo), GST_VIDEO_INFO_HEIGHT (&vinfo));

  new_caps = gst_video_info_to_caps (&conv_vinfo);

  /* create a new pool for the new configuration */
  newpool =
      gst_kms_sink_create_pool (self, new_caps,
      GST_VIDEO_INFO_SIZE (&conv_vinfo), 3);
  if (!newpool)
    goto no_pool;
  ...
  if (!gst_kms_sink_check_scale (self, &conv_vinfo))
    goto invalid_format;
...
static gboolean
gst_rga_sink_import_dmabuf (GstRGASink * self, GstBuffer * inbuf,
    GstBuffer ** outbuf)
{
  ...
  switch (GST_VIDEO_INFO_FORMAT (&self->vinfo)) {
    ...
    case GST_VIDEO_FORMAT_NV21:
      fmt = RK_FORMAT_YCrCb_420_SP;
      break;
    case GST_VIDEO_FORMAT_I420:
      fmt = RK_FORMAT_YCbCr_420_P;
      break;
    default:
      GST_ERROR_OBJECT (self, "don't support this format %s",
          gst_video_format_to_string (GST_VIDEO_INFO_FORMAT (&self->vinfo)));
      goto rga_failed;
  }

  rga_set_rect (&src_info.rect, 0, 0, GST_VIDEO_INFO_WIDTH (&self->vinfo),
      GST_VIDEO_INFO_HEIGHT (&self->vinfo),
      GST_VIDEO_INFO_PLANE_STRIDE (&self->vinfo, 0),
      GST_VIDEO_INFO_PLANE_OFFSET (&self->vinfo, 1) /
      GST_VIDEO_INFO_PLANE_STRIDE (&self->vinfo, 0), fmt);

  dmamem = gst_buffer_peek_memory (*outbuf, 0);
  dst_info.fd = gst_dmabuf_memory_get_fd (dmamem);
  dst_info.mmuFlag = 1;

  rga_set_rect (&dst_info.rect, 0, 0, GST_VIDEO_INFO_WIDTH (&self->conv_vinfo),
      GST_VIDEO_INFO_HEIGHT (&self->conv_vinfo),
      GST_VIDEO_INFO_WIDTH (&self->conv_vinfo),
      GST_VIDEO_INFO_HEIGHT (&self->conv_vinfo), RK_FORMAT_RGBX_8888);

  if (c_RkRgaBlit (&src_info, &dst_info, NULL))
    goto rga_failed;

  kmsmem = (GstKMSMemory *) gst_kms_allocator_get_cached (dmamem);
  if (kmsmem) {
...
rga_failed:
  GST_ERROR_OBJECT (self, "RGA can't process the image");
  return FALSE;
}

static GstBuffer *
gst_rga_sink_copy_to_dumb_buffer (GstRGASink * self,
    GstBuffer * inbuf, GstBuffer * buf)
{
  ...
  if (!gst_video_frame_map (&outframe, &self->conv_vinfo, buf, GST_MAP_WRITE))
...
static GstBuffer *
gst_rga_sink_get_input_buffer (GstRGASink * self, GstBuffer * inbuf)
{
  GstMemory *mem;
  GstBuffer *buf = NULL;
  GstFlowReturn ret;
  ...
  if (gst_rga_sink_import_dmabuf (self, inbuf, &buf))
    goto done;

  GST_CAT_INFO_OBJECT (CAT_PERFORMANCE, self, "frame copy");
  buf = gst_rga_sink_copy_to_dumb_buffer (self, inbuf, buf);
...
gst_kms_sink_show_frame (GstVideoSink * vsink, GstBuffer * buf)
{
  ...
  self = GST_RGA_SINK (vsink);

  res = GST_FLOW_ERROR;

  if (buf)
    buffer = gst_rga_sink_get_input_buffer (self, buf);
  ...
  if ((crop = gst_buffer_get_video_crop_meta (buffer))) {
    GstVideoInfo vinfo = self->conv_vinfo;
  ...
  if (crop) {
    ...
    src.w = GST_VIDEO_INFO_WIDTH (&self->conv_vinfo);
    src.h = GST_VIDEO_INFO_HEIGHT (&self->conv_vinfo);
  }
...
static void
gst_kms_sink_drain (GstRGASink * self)
{
...
{
  GstRGASink *self = GST_RGA_SINK (bsink);
...
static void
gst_rga_sink_init (GstRGASink * sink)
{
  sink->fd = -1;
  sink->conn_id = -1;
  sink->plane_id = -1;
...
static void
gst_rga_sink_class_init (GstRGASinkClass * klass)
{
  ...
  gst_element_class_set_static_metadata (element_class, "RGA video sink",
      "Sink/Video", GST_PLUGIN_DESC, "Randy Li <randy.li@rock-chips.com>");
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, rga,
